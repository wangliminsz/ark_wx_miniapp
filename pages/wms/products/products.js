...
Page({
  ...
  onReachBottom() {
    ...
    if (d_current_page <= d_total_pages) {
      ...
      wx.showLoading({ title: 'Loading more...' }); // Add loading indicator
      ...
      this.setData({ loading: false }); // Reset loading on error
      wx.hideLoading(); // Hide loading on failure
      console.error('Failed to load more records:', error);
      wx.showToast({ title: 'Load failed', icon: 'none' }); // Optional: show error
    });
