...
Page({
  ...
  // 111 async cloudDbRead(openid) {
  //   // Get the cloud instance
  //   const cloud = getApp().cloud; // Assuming you've attached the cloud instance to the global app

  //   // Use the cloud instance to get the database
  ...
  updateUserInfo2: function () {
    wx.navigateTo({
      ...
      success: function (res) {
        console.log('Navigation to /login/register successful');
      },
      fail: function (err) {
        console.error('Navigation to /login/register failed', err);
      }
    });
  },
