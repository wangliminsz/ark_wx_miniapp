...
Page({
  ...
  data: {
    ...
    oriUrl: '',
    filterUrl: '',
    cleanUrl: `${config.fastapiUrl}/th_ant_new?`,
  ...
  // // https://mp.weixin.qq.com/s/rQxqmHire85QDDGL5Rh_IQ'
  // showArticle: function(wxUrl) {
  //   // this.setData({
  //   //   showWebView: true
  //   // });
  //   wx.navigateTo({
  //     url: '/pages/listnew/listnew_article?articleUrl=' + encodeURIComponent(wxUrl)
  //   });
  // },
  ...
  async loadData(loadUrl, current_page) {
    ...
    try {
      ...
      console.log('2025 ---> gRecords', gRecords_data)
      if (gRecords) {
        this.setData({
  ...
  async onLoad(options) {
    ...
    console.log('listpage 2025 --->>>', options)
    if (options.url) {
      ...
      this.setData({
        ...
        this.setData({
          ...
          console.log('mycity mytype keyword--->', this.data.mycity, this.data.keyword)
        });
      })
    }
  ...
  handleMyViewTap(e) {
    ...
    if (itemData) {
      wx.navigateTo({
        url: '/pages/detailnew/detailnew?id=' + itemId,
        success: (res) => {
  ...
  onShareAppMessage: function () {
    ...
    return {
      title: 'Ant Global',
      path: `/pages/listnew/listnew?url=${thisUrl ? thisUrl : ''}`
    };
  },
